...
class ModelController:
    ...
    def __init__(self, image_size, latent_size, batch_size, sequence_size,
        ...
        self.dataset_multi_ctrl = DatasetController_multi_object(batch_size=int(batch_size), sequence_size=1)
        ...
        images_att, images, _, _, _, _, _, _, _, _, _ , _, _, _, objects, objs_one_hot, describtions, descs_one_hot, _, _ = next(self.generator_test)
        images_att_multi, images_multi, _, _, objs_multi, objs_one_hot_multi, descs_multi, descs_one_hot_multi, _, _ = next(self.generator_test_multi)
        ...
        self.sample_objs_one_hot = np.concatenate((temp, np.squeeze(np.asarray(objs_one_hot_multi[:sample_size], dtype=np.float32))), axis=0)
        temp = np.asarray(descs_one_hot[:sample_size], dtype=np.float32)
        self.sample_descs_one_hot = np.concatenate((temp, np.squeeze(np.asarray(descs_one_hot_multi[:sample_size], dtype=np.float32))), axis=0)

        temp = np.asarray(objects[:sample_size], dtype=np.float32)
        objects = np.concatenate((temp, np.squeeze(np.asarray(objs_multi[:sample_size], dtype=np.float32))), axis=0)
        temp = np.asarray(describtions[:sample_size], dtype=np.float32)
        describtions = np.concatenate((temp, np.squeeze(np.asarray(descs_multi[:sample_size], dtype=np.float32))), axis=0)
    ...
    def train(self):
        ...
        for epoch in xrange(1, self.epoch_num + 1):
            ...
            while batches_passed < 1000:
                ...
                att_images, images, _, _, _, _, _, _, _, _, joints, _, batch_one_hot, sentence, objs, objs_one_hot, descs, descs_one_hot, _, _ = next(self.generator)
                # att_images_multi, images_multi, _, sentence_multi, objs_multi, objs_one_hot_multi, descs_multi, descs_one_hot_multi = next(self.generator_multi)
                ...
                for k, g in enumerate(GPU.gpus_to_use):
                    ...
                    self.batch_gpu_threads[k] = threading.Thread(target=self.handle_gpu_batch, args=(
                        batches_passed, batch_start_time, k, g, images, att_images, None, joints, batch_one_hot, gt_vocab, objs, objs_one_hot, descs, descs_one_hot))
    ...
    def handle_gpu_batch(self, batches_passed, batch_start_time, k, g, images, att_images, secondary_images, joints, batch_one_hot, gt_vocab, objects, objs_one_hot, describtions, describtions_one_hot):
        ...
        describtions = np.repeat(describtions[np.newaxis], self.sequence_size, axis=0)
        describtions = np.squeeze(np.reshape(describtions, (self.sequence_size * gpu_batch_size, -1)))

        objects_norm = np.asarray(objects, dtype=np.float32)
        describtions_norm = np.asarray(describtions, dtype=np.float32)
        ...
        # reconstruction_loss_whole += F.mean_squared_error(x00_whole[0::2], x_in[0::2])
        # reconstruction_loss_att += F.mean_squared_error(x00_att_att, x_in_att)
        ...
        # l_dis_obj_rec_att2 = F.softmax_cross_entropy(y0_att2, Variable(cuda.to_gpu(xp.zeros(gpu_images_size).astype(np.int32), g)))
        # l_dis_desc_rec_att2 = F.softmax_cross_entropy(d0_att2, Variable(cuda.to_gpu(xp.zeros(gpu_images_size).astype(np.int32), g)))

        # l_dis_obj_rec_whole_00 = F.softmax_cross_entropy(y00_whole, Variable(cuda.to_gpu(xp.zeros(gpu_images_size).astype(np.int32), g)))
        # l_dis_desc_rec_whole_00 = F.softmax_cross_entropy(d00_whole, Variable(cuda.to_gpu(xp.zeros(gpu_images_size).astype(np.int32), g)))
        # l_dis_obj_rec_whole_00_2 = F.softmax_cross_entropy(y00_whole2, Variable(cuda.to_gpu(xp.zeros(gpu_images_size).astype(np.int32), g)))
        # l_dis_desc_rec_whole_00_2 = F.softmax_cross_entropy(d00_whole2, Variable(cuda.to_gpu(xp.zeros(gpu_images_size).astype(np.int32), g)))
        ...
        # l_dis_0 += (l_dis_obj_rec_whole_00 + l_dis_desc_rec_whole_00 + l_dis_obj_rec_whole_00_2 + l_dis_desc_rec_whole_00_2) / (4 * gpu_images_size)
        ...
        # l_gen_obj_sim_att2 = F.softmax_cross_entropy(y0_att2, objects_var)
        # l_gen_desc_sim_att2 = F.softmax_cross_entropy(d0_att2, desc_var)

        # l_gen_obj_sim_whole_00 = F.softmax_cross_entropy(y00_whole, objects_var)
        # l_gen_desc_sim_whole_00 = F.softmax_cross_entropy(d00_whole, desc_var)
        # l_gen_obj_sim_whole_00_2 = F.softmax_cross_entropy(y00_whole2, objects_var)
        # l_gen_desc_sim_whole_00_2 = F.softmax_cross_entropy(d00_whole2, desc_var)
        ...
        # z12 = Variable(cuda.to_gpu(xp.random.normal(0, 1, (gpu_images_size, self.latent_size), dtype=np.float32), g))
        ...
        # x12_whole = self.gen_models_att[k](z12, Variable(cuda.to_gpu(objects_one_hot, g)), Variable(cuda.to_gpu(descs_one_hot, g)), train=self.train_autoencoder)
        ...
        # y2, d2, _ = self.dis_models[k](x_in[0::2], att=False, train=self.train_dis)
        # y2, d2, l2_whole = self.dis_models[k](x_in, att=False, train=self.train_dis)
        ...
        # l_dis_obj_real = F.softmax_cross_entropy(y2, objects_var[0::2]) 
        # l_dis_desc_real = F.softmax_cross_entropy(d2, desc_var[0::2])
        # l_dis_obj_real = F.softmax_cross_entropy(y2, objects_var) 
        # l_dis_desc_real = F.softmax_cross_entropy(d2, desc_var)
        ...
        #l_dis_obj_real_sec = F.softmax_cross_entropy(y2_sec, objects_var)
        #l_dis_desc_real_sec = F.softmax_cross_entropy(d2_sec, desc_var)
    ...
    def save_models(self):
        ...
        if test_lstm_loss < self.last_best_result:
            # self.last_best_result = test_lstm_loss
    ...
    def test_testset(self):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            ...
            for i in range(num_batches):
                att_images, images, _, _, _, _, _, _, _, _, joints, _, batch_one_hot, _, _, obj_one_hot, _, descs_one_hot, _, _ = next(self.generator_test)
                # att_images_multi, images_multi, _, _, objs_multi, objs_one_hot_multi, descs_multi, descs_one_hot_multi = next(self.generator_test_multi)
    ...
    def save_sample_images(self, epoch, batch):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            ...
            # self.save_image(cuda.to_cpu(data_whole), 'sample/{0:03d}_{1:07d}_whole.png'.format(epoch, batch))
            # image = ((cuda.to_cpu(data_att) + 1) * 128).clip(0, 255).astype(np.uint8)
            # image = image[:self.sample_image_rows * self.sample_image_cols]
            # image = image.reshape(
            #     (self.sample_image_rows, self.sample_image_cols, self.num_channels, self.image_size,
            #      self.image_size)).transpose(
            ...
            #     image = image.reshape(self.sample_image_rows * self.image_size,
            #                           self.sample_image_cols * self.image_size)
            # Image.fromarray(image).save('sample/{0:03d}_{1:07d}_att.png'.format(epoch, batch))

            # image = ((cuda.to_cpu(data_whole) + 1) * 128).clip(0, 255).astype(np.uint8)
            # image = image[:self.sample_image_rows * self.sample_image_cols]
            # image = image.reshape(
            #     (self.sample_image_rows, self.sample_image_cols, self.num_channels, self.image_size,
            #      self.image_size)).transpose(
            ...
            #     image = image.reshape(self.sample_image_rows * self.image_size,
            #                           self.sample_image_cols * self.image_size)
            # Image.fromarray(image).save('sample/{0:03d}_{1:07d}_whole.png'.format(epoch, batch))
            if batch == 0:
                self.save_image(images, 'sample/org_pic.png')
                self.save_image(images_att, 'sample/org_att.png')
                # org_image = ((np.add(images, 1)) * 128).clip(0, 255).astype(np.uint8)
                # org_image_att = ((np.add(images_att, 1)) * 128).clip(0, 255).astype(np.uint8)
                # org_image = org_image.reshape(
                #     (self.sample_image_rows, self.sample_image_cols, self.num_channels, self.image_size,
                #      self.image_size)).transpose(
                #     (0, 3, 1, 4, 2)).reshape(
                #     (self.sample_image_rows * self.image_size, self.sample_image_cols * self.image_size,
                #      self.num_channels))
                # org_image_att = org_image_att.reshape(
                #     (self.sample_image_rows, self.sample_image_cols, self.num_channels, self.image_size,
                #      self.image_size)).transpose(
                #     (0, 3, 1, 4, 2)).reshape(
                #     (self.sample_image_rows * self.image_size, self.sample_image_cols * self.image_size,
                #      self.num_channels))
                # if self.num_channels == 1:
                #     org_image = org_image.reshape(self.sample_image_rows * self.image_size,
                #                                   self.sample_image_cols * self.image_size)
                # Image.fromarray(org_image).save('sample/org_pic.png'.format(epoch, batch))
                # Image.fromarray(org_image_att).save('sample/org_att.png'.format(epoch, batch))
