...
class EncodeDataset:
    def __init__(self):
        ...
        self.enc_model = autoencoder.Encoder_text_tower(density=8, size=image_size, latent_size=latent_size, channel=self.num_channels, num_objects=self.num_objects, num_describtions=self.num_descs)
    ...
    def encode(self, image, object_involved_one_hot, describtors_involved_one_hot):
        xp = cuda.cupy
        ...
        describtors_involved_one_hot = np.repeat(describtors_involved_one_hot, x_in.shape[0], axis=0)
        z0, mean, var = self.enc_model(Variable(x_in), Variable(cuda.to_gpu(object_involved_one_hot, GPU.main_gpu)), Variable(cuda.to_gpu(describtors_involved_one_hot, GPU.main_gpu)) ,train=False)
        # print z0.shape
        return F.squeeze(z0)
    ...
    def process_tasks(self, dir_name):
        ...
        for subdir_name in os.listdir(source):
            if subdir_name in self.DC.reverse_annotations[int(dir_name)].keys():
                ...
                self.process_demonstration(os.path.join(source, subdir_name), dest_subdir_path, object_involved_one_hot, describtors_involved_one_hot)

    def process_demonstration(self, dir_path, dest_dir_path, object_involved_one_hot, describtors_involved_one_hot):
        dem_folder = dir_path
        ...
        for camera in self.cameras:
            _, valid_ts = self.images_to_latent(dem_folder, camera, object_involved_one_hot, describtors_involved_one_hot, timestamps=timestamps_robot, verify=True)
        ...
        for camera in self.cameras_to_process:
            if not os.path.exists(os.path.join(dest_dir_path, camera + '.npy')):
                latents, _ = self.images_to_latent(dem_folder, camera, object_involved_one_hot, describtors_involved_one_hot ,timestamps=timestamps_robot, verify=False)
                np.save(os.path.join(dest_dir_path, camera + '.npy'), latents)
        # if not os.path.exists(os.path.join(dest_dir_path, 'cameras.npy')):
        #     latents, _ = self.all_camera_images_to_latent(dem_folder, timestamps=timestamps_robot, verify=False)
        #     np.save(os.path.join(dest_dir_path, 'cameras.npy'), latents)

    def images_to_latent(self, path, camera, object_involved_one_hot, describtors_involved_one_hot, timestamps=None, isRobot='robot', verify=False):
        valid_ts = []
        ...
        if not verify:
            if images.shape[0] > self.batch_size:
                for j in range(0, images.shape[0] - (images.shape[0] % self.batch_size), self.batch_size):
                    ext = self.encode(images[j:j + self.batch_size], object_involved_one_hot, describtors_involved_one_hot)
                    latents[j:j + self.batch_size] = cuda.to_cpu(ext.data)

                j += self.batch_size
                if j < images.shape[0]:
                    ext = self.encode(images[j:], object_involved_one_hot, describtors_involved_one_hot)
                    latents[j:] = cuda.to_cpu(ext.data)
            else:
                ext = self.encode(images,  object_involved_one_hot, describtors_involved_one_hot)
