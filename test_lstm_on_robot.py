...
class ModelTester:
    ...
    def __init__(self, image_size, latent_size, output_size=7, num_channels=3, task='5002', tasks=['5001', '5002', '5003', '5004'], save_dir="model/"):
        ...
        self.enc_model = autoencoder.Encoder_text_tower(density=8, size=self.image_size, latent_size=self.latent_size,
                                             channel=self.num_channels, num_objects=self.num_objects, num_descriptions=self.num_descs)
        ...
        self.descriptions_norm[0] = (float(self.which_describtor) / (self.num_descs + 1)) - 1
        self.descriptions_norm = self.descriptions_norm[:, np.newaxis, np.newaxis, np.newaxis]
        self.descriptions_norm = np.tile(self.descriptions_norm, (1, 1, self.image_size, self.image_size))
